...
serve(async (req)=>{
  ...
  try {
    ...
    let packageQuery = supabaseClient.from('consultation_packages').select('*').eq('is_active', true);
    if (packageId) {
    ...
    if (isNigeria) {
      ...
      const { data: regionalPricing, error: regionalError } = await supabaseClient.from('regional_consultation_pricing').select('*').eq('package_id', consultationPackage.id).eq('country_code', NIGERIA_COUNTRY_CODE).eq('is_active', true).limit(1);
