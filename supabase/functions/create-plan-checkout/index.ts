...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: plans, error: plansError } = await supabaseClient.from('subscription_plans').select('*').eq('tier', tier).eq('is_active', true).limit(1);
    ...
    if (isNigeria) {
      ...
      const { data: regionalPricing, error: regionalError } = await supabaseClient.from('regional_pricing').select('*').eq('plan_id', plan.id).eq('country_code', NIGERIA_COUNTRY_CODE).eq('is_active', true).limit(1);
