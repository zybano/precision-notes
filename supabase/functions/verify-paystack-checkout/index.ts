...
serve(async (req)=>{
  ...
  try {
    ...
    const { error: updateError } = await supabase.from('transaction_history').update({
      status: 'completed',
    ...
    if (productType === 'consultation') {
      ...
      const { data: userData, error: userError } = await supabase.from('user_credits').select('balance, total_earned').eq('user_id', userId).single();
      ...
      if (userData) {
        const { error: creditUpdateError } = await supabase.from('user_credits').update({
          balance: newBalance,
        ...
        const { error: creditInsertError } = await supabase.from('user_credits').insert({
          user_id: userId,
          balance: quantity,
      ...
      if (metadata.package_id) {
        const { error: purchaseError } = await supabase.from('consultation_purchases').insert({
          user_id: userId,
      ...
      const { data: existingSub, error: subQueryError } = await supabase.from('user_subscriptions').select('*').eq('user_id', userId).single();
      ...
      if (existingSub) {
        ...
        const { error: subInsertError } = await supabase.from('user_subscriptions').insert({
          user_id: userId,
          subscription_tier: tier,
          is_annual_billing: billingCycle === 'annual',
          next_billing_date: nextBillingDate.toISOString(),
