...
serve(async (req) => {
  ...
  try {
    ...
    const { to, subject, html, fromEmail, fromName, cc, bcc, replyTo, bounceAddress, clientReference, mimeHeaders } = body;
    ...
    console.error("send-email-html error", error);
...
async function sendHtmlEmail(options: {
  ...
  return callZeptoMail("/email", payload);
}
