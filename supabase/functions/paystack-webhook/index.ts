...
serve(async (req)=>{
  ...
  try {
    ...
    const signature_bytes = await crypto.subtle.sign("HMAC", key, encoder.encode(rawBody));
    // Convert to hex
    ...
    if (payload.event === "charge.success") {
      ...
      if (productType === "consultation") {
        ...
        const { data: existingSub } = await supabase.from("user_subscriptions").select("*").eq("user_id", userId).single();
        if (existingSub) {
          ...
          const consultationsTotal = tier === "free" ? 10 : tier === "starter" ? 50 : tier === "professional" ? 200 : 500;
